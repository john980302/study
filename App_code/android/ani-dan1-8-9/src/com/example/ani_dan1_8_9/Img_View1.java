package com.example.ani_dan1_8_9;

import java.util.HashMap;
import java.util.Random;

import com.example.ani_dan1_8_9.R;
...
public class Img_View1 extends View{
	...
	public Img_View1(Context context){
		...
		setEffects(_context,2); // 사운드재생
		addSound(_context,0,R.raw.left); //사운드불러오기
		...
		if(event.getAction() == MotionEvent.ACTION_DOWN || event.getAction() == MotionEvent.ACTION_MOVE){
			...
			if(rt.contains(xx,yy)){
				...
				if(event.getAction() == MotionEvent.ACTION_DOWN){
					//0번 효과음 사운드 1회재생함 2배속재생(0.5는 2배 느려짐)
					//버튼을 빠르게 눌러보자
					playSound(0,1,2f);
				}
			}
