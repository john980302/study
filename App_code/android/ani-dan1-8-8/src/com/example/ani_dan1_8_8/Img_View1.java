...
public class Img_View1 extends View{
	...
	//사운드풀 객체를 생성
	//사운드풀은 각종 효과음 재생에 효과적이다.
	//긴 배경음은 MediaPlayer로 재생하자
	//3: 3개의 효과음을 사용한다
	//AudioManager.STREAM_MUSIC: 보통 게임효과음의 경우 선택한다.
	//0: 재생 품질로 보통 0을 사용
	SoundPool msoundpool = new SoundPool(3, AudioManager.STREAM_MUSIC,0);
	int sound_left;
	int sound_right;
	int play_left;
	int play_right;
	...
	// 쓰레드를 일시정지 할 수 있도록 만든 변수
	boolean thread1_pause=false;
	
	// 쓰레드 setRunning함수에 private boolean m_run=false; 변수 추가로 아래 retry변수 제거함
	//boolean retry = true;
	int ch_cnt1 = 0;
	...
	public Img_View1(Context context){
		...
		//_context: 현재 뷰의 객체이다. 뷰가 아닌 액티비티라면 this로 기록가능
		//R.raw.left: 왼쪽 사운드 리소스 파일이름
		//R.raw.right: 오른쪽 사운드 리소스 파일이름
		sound_left = msoundpool.load(_context, R.raw.left,1);
		sound_right = msoundpool.load(_context, R.raw.right,1);
		...
		// 쓰레드 setRunning함수 추가해서 쓰레드 종료시 보이는 앱종료 메세지 제거
	...
	public void onDetachedFromWindow(){
		//뷰를 종료할 때 효과음을 모두 멈춤
		msoundpool.stop(play_left);
		msoundpool.stop(play_right);
		...
		//쓰레드 setRunning함수 추가해서 쓰레드 종료시 보이는 앱종료 메세지 제거
		//false시 쓰레드 종료함
		thread1.setRunning(false); // thread1 종료
		while(retry1){
			try{
	...
	// 뷰가 화면에서 사라지고 보일때 처리하기
	// 홈버튼을 선택하면 뷰가 스마트폰에서 사라진다.
	// 이때 발생되는 이벤트이다.
	@Override
	public void onWindowVisibilityChanged(int visibility){
		super.onWindowVisibilityChanged(visibility);
		if(visibility==View.VISIBLE){ //뷰가 화면에서 보이면
			backsound.start(); // 배경음악을 재생
			thread1_pause=false;
		}else{ //뷰가 화면에서 사라지면 아래 명령을 처리한다.
			backsound.pause();// 배경음악을 일시정지
			thread1_pause=true;
		}
	}
		...
		if(event.getAction() == MotionEvent.ACTION_DOWN || event.getAction() == MotionEvent.ACTION_MOVE){
			...
			if(rt.contains(xx,yy)){
				...
				if(event.getAction() == MotionEvent.ACTION_DOWN){
					//사운드풀로 효과음을 재생
					//버튼을 빠르게 눌러보자
					//MediaPlayer는 고속시 끊김현상이 일어남
					//SoundPool방식은 빠르게 선택해도 모든 소리가 재생됨
					//sound_left: 왼쪽버튼 선택 효과음 재생
					//첫번째 1.0F: 왼쪽볼륨으로 실수형태로 전달 (0이 가장낮고 1이 가장크다)
					//두번째 1.0F: 오른쪽볼륨으로 실수형태로 전달
					//1: 사운드재생 우선순위이다. 동시에 소리를 출력할 때 우선순위로 0이 가장 낮다.
					//0: 반복모드로 0이면 1회 1이면 2회 -1이면 무한반복
					//2.0F: 재생속도로 2배속 재생이다.(정상:1,2배느려짐:0.5,2배빠르게:2)
					play_left = msoundpool.play(sound_left, 1.0F, 1.0F, 0, 0, 2.0F);
			...
			if(rt.contains(xx,yy)){
				...
				//버튼을 눌렀을 때만 재생하도록 했다. ACTION_MOVE 즉, 터치후 움직일 때도 소리가 계속 들린다.
				if(event.getAction() == MotionEvent.ACTION_DOWN){
					play_right = msoundpool.play(sound_right, 1.0F, 1.0F, 0, 0, 2.0F);
					//btnsound.seekTo(0); //버튼을 누르면 효과음의 처음부터 소리가 재생되도록한다.
					//btnsound.start(); // 소리를 재생한다.
				}
			}
	...
	class BackThread extends Thread{
		//쓰레드 동작에 사용할 m_run변수를 선언
		private boolean m_run=false;
		//쓰레드 setRunning함수 추가해서 인수값  run을 통해 동작 true, 정지 false값을 받는다.
		...
		public void run(){
			try{
				while(m_run){
					try{
						// 쓰레드 일시정지 변수 thread1_pause가 false이면 동작하고 true이면 동작을 멈춤
						// 홈버튼을 선택하면 게임진행이 모두 일시정지, 소리도 포함
						// 홈버튼 선택시 thread1_pause를 true로 해서 게임의 동작을 멈추고
						// 해당게임이 스마트폰에서 제거되지 않고 대기상태에서 다시실행하면
						// thread1_pause를 다시 false로 해서 다시 동작하도록 한다.
